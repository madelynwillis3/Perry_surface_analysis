...
generate_spatial_bootstrap <- function(sf_data, n_boot = 50, train_frac = 0.7, n_clusters = 10, seed = 123){
  set.seed(seed)
  coords <- st_coordinates(sf_data)
  clusters <- kmeans(coords, centers = n_clusters)$cluster
  boot_list <- vector("list", n_boot)
  for(i in 1:n_boot){
    train_idx <- unlist(lapply(split(1:nrow(sf_data), clusters), function(idx){
      sample(idx, size = ceiling(length(idx)*train_frac))
    }))
    df <- sf_data %>% mutate(set = ifelse(1:nrow(sf_data) %in% train_idx, "train", "test"))
    boot_list[[i]] <- df
  }
  return(boot_list)
}
...
# --- Generate bootstraps ---
boot_SP <- generate_spatial_bootstrap(soil_sf_SP, n_boot = 50, n_clusters = 8)
boot_C  <- generate_spatial_bootstrap(soil_sf_C, n_boot = 50, n_clusters = 8)
  ...
  for(var in soil_vars){
    boot_results <- map(boot_list, function(df){
      ...
      if(all(is.na(train_vals)) || length(unique(train_vals)) <=1){
        ...
        return(list(pred=pred, RMSE=sqrt(mean((pred - test_vals)^2, na.rm=TRUE)), R2=safe_cor2(pred,test_vals)))
      }
      vgm_model <- tryCatch(variogram(as.formula(paste(var,"~1")), train_sf) %>%
                              fit.variogram(vgm(psill=var(train_vals, na.rm=TRUE),
                                                 model="Exp", nugget=0.1, range=100)),
                            error=function(e) NULL)
      if(is.null(vgm_model)) vgm_model <- vgm(psill=var(train_vals, na.rm=TRUE),
                                              model="Exp", nugget=0.1, range=100)
      ok_pred <- tryCatch(krige(as.formula(paste(var,"~1")), train_sf, test_sf, model=vgm_model),
                          error=function(e) NULL)
      if(is.null(ok_pred)) ok_pred <- data.frame(var1.pred=rep(mean(train_vals, na.rm=TRUE), nrow(test_sf)))
      rmse <- sqrt(mean((ok_pred$var1.pred - test_vals)^2, na.rm=TRUE))
      r2   <- safe_cor2(ok_pred$var1.pred, test_vals)
      list(pred=ok_pred$var1.pred, RMSE=rmse, R2=r2)
    })
    results[[var]] <- boot_results
  }
  return(results)
